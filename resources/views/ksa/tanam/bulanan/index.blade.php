...
@section('title', 'KSA Sanding Bulanan Luas Tanam Okt ' . $tahun . ' – Sep ' . ($tahun + 1))
...
<div class="max-w-full mx-auto px-4 py-6">
    ...
    <div class="mb-6 flex items-center justify-between flex-wrap gap-4">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 flex items-center gap-3">
                ...
                LTT Padi Sanding Oktober {{ $tahun }} – September {{ $tahun + 1 }}
            </h1>
            <p class="mt-2 text-sm text-gray-600">KSA &mdash; Input data per kabupaten per bulan &bull; Satuan: Hektar (Ha)</p>
        </div>
        <div class="flex gap-2 flex-wrap">
            @if(Auth::user()->canManageData())
            <a href="{{ route('ksa.tanam.bulanan.create', ['tahun' => $tahun]) }}"
               class="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-lg hover:from-orange-600 hover:to-orange-700 shadow-lg hover:shadow-xl transition-all duration-300 font-semibold">
            ...
            @endif
            <a href="{{ route('ksa.tanam.bulanan.export', ['tahun' => $tahun]) }}"
        ...
        <div class="bg-white rounded-xl shadow-md p-5 border border-gray-200">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="flex flex-col">
                    <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Tahun (Periode Oktober)</label>
                    <select name="tahun" class="w-full h-10 border-gray-300 rounded-lg shadow-sm focus:border-orange-500 focus:ring-2 focus:ring-orange-200 transition-all text-sm">
                        @foreach($availableYears as $y)
                            <option value="{{ $y }}" {{ $y == $tahun ? 'selected' : '' }}>Okt {{ $y }} – Sep {{ $y + 1 }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="flex flex-col">
                ...
                <div class="flex flex-col">
                    ...
                    <div class="flex gap-2 h-10">
                        ...
                        <a href="{{ route('ksa.tanam.bulanan.index') }}" class="h-10 px-4 flex items-center justify-center bg-gray-100 text-gray-600 rounded-lg hover:bg-gray-200 font-semibold text-sm transition-all whitespace-nowrap">
                            Reset
                        </a>
                    </div>
                </div>
    ...
    <div class="bg-gradient-to-r from-orange-50 to-amber-50 border-l-4 border-orange-500 p-4 mb-6 rounded-r-lg shadow-md">
        <div class="flex items-center gap-3">
            ...
            <p class="text-sm text-orange-800 font-medium">
                <strong>Sanding Bulanan Periode Okt {{ $tahun }} – Sep {{ $tahun + 1 }}</strong> &mdash; Data ini otomatis muncul di
                <a href="{{ route('ksa.tanam.index') }}" class="underline font-bold">Sanding Tahunan per Bulan</a> dan
                <a href="{{ route('ksa.tanam.total.index') }}" class="underline font-bold">Sanding Total Tahunan</a>.
            ...
            <table class="min-w-full text-xs border-collapse">
                <thead>
                    <tr class="bg-gradient-to-r from-orange-500 to-orange-600 text-white">
                        ...
                        <th class="border border-orange-600 px-4 py-2 text-center font-bold" colspan="12">LTT Padi (Ha) Tahun {{ $tahun }}</th>
                        <th class="border border-orange-600 px-4 py-3 text-center font-bold min-w-[100px]" rowspan="2">Total</th>
                        @if(Auth::user()->canManageData())
                        <th class="border border-orange-600 px-3 py-3 text-center font-bold" rowspan="2">Aksi</th>
                        @endif
                    </tr>
                    <tr class="bg-gradient-to-r from-orange-400 to-orange-500 text-white text-xs">
                        @foreach($bulanList as $b => $nama)
                        <th class="border border-orange-500 px-2 py-2 text-center font-bold min-w-[64px]">{{ substr($nama,0,3) }}</th>
                        @endforeach
                    </tr>
                </thead>
                <tbody class="bg-white">
                    ...
                    @php
                        ...
                        $totalKab = $kabRows->sum('luas_tanam');
                    @endphp
                    <tr class="hover:bg-orange-50 transition-colors duration-150 {{ $i % 2 == 0 ? 'bg-gray-50' : 'bg-white' }}">
                        ...
                        @foreach($bulanList as $b => $nama)
                        @php $val = $kabRows->firstWhere('bulan', $b)?->luas_tanam ?? null; @endphp
                        <td class="border border-gray-200 px-2 py-2.5 text-right {{ $val > 0 ? 'text-gray-900 font-medium' : 'text-gray-300' }}">
                            {{ fmtVal($val) }}
                        </td>
                        @endforeach

                        <td class="border border-orange-200 px-3 py-2.5 text-right font-bold {{ $totalKab > 0 ? 'text-orange-700 bg-orange-50' : 'text-gray-300' }}">
                            {{ fmtVal($totalKab) }}
                        </td>
                        ...
                        <td class="border border-gray-200 px-2 py-2 text-center whitespace-nowrap">
                            ...
                            <div class="flex items-center justify-center gap-1">
                                <a href="{{ route('ksa.tanam.bulanan.edit', ['kabupaten_id' => $kab->id, 'tahun' => $tahun]) }}"
                                   class="inline-flex items-center px-2 py-1 bg-orange-500 text-white rounded hover:bg-orange-600 text-xs font-semibold transition-colors">
                                ...
                                <form method="POST" action="{{ route('ksa.tanam.bulanan.destroy', ['kabupaten_id' => $kab->id]) }}" class="inline"
                                      onsubmit="return confirm('Hapus semua data {{ addslashes($kab->nama_kabupaten) }} periode Okt {{ $tahun }}–Sep {{ $tahun + 1 }}?')">
                                    @csrf @method('DELETE')
                ...
                <tfoot>
                    <tr class="bg-gradient-to-r from-orange-700 to-orange-800 text-white font-bold">
                        ...
                        <td class="border border-orange-800 px-2 py-3 text-right font-extrabold">
                            {{ fmtVal($tot) }}
                        </td>
                        @endforeach
                        <td class="border border-orange-800 px-3 py-3 text-right font-extrabold bg-orange-900">
                            @php $gs = $totalPerBulan->sum(); @endphp
                            {{ fmtVal($gs) }}
                        </td>
                        @if(Auth::user()->canManageData())
                        <td class="border border-orange-800 px-2 py-3"></td>
                        @endif
                    </tr>
                </tfoot>
            </table>
    ...
    <p class="mt-3 text-xs text-gray-400">* Satuan: Hektar (Ha) &nbsp;|&nbsp; Periode: Oktober {{ $tahun }} – September {{ $tahun + 1 }}</p>

    {{-- ── SUMMARY CARDS ── --}}
    <div class="mt-6 grid grid-cols-1 md:grid-cols-4 gap-4">

        {{-- Card 1: orange-500 → orange-600 --}}
        <div class="bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl p-6 text-white shadow-xl">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold mt-2">{{ $filteredKabupatens->count() }}</p>
                </div>
        ...
        <div class="bg-gradient-to-br from-amber-500 to-amber-700 rounded-xl p-6 text-white shadow-xl">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs mt-1 opacity-75">Okt {{ $tahun }}–Sep {{ $tahun + 1 }}</p>
                </div>
                <div class="bg-white/20 p-3 rounded-lg">
