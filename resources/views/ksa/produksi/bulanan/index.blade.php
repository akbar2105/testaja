...
@section('title', 'KSA Sanding Bulanan Produksi Padi ' . $tahun)
...
<div class="max-w-full mx-auto px-4 py-6">
    ...
    <div class="mb-6 flex items-center justify-between flex-wrap gap-4">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 flex items-center gap-3">
                ...
                KSA Produksi Padi Sanding Bulanan Tahun {{ $tahun }}
            </h1>
            <p class="mt-2 text-sm text-gray-600">KSA &mdash; Input data produksi per kabupaten per bulan &bull; Satuan: Ton GKG</p>
        </div>
        <div class="flex gap-2 flex-wrap">
            @if(Auth::user()->canManageData())
            <a href="{{ route('ksa.produksi.bulanan.create', ['tahun' => $tahun]) }}"
               class="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-lg hover:from-blue-600 hover:to-indigo-700 shadow-lg hover:shadow-xl transition-all duration-300 font-semibold">
            ...
            @endif
            <a href="{{ route('ksa.produksi.bulanan.export', ['tahun' => $tahun]) }}"
        ...
        <div class="bg-white rounded-xl shadow-md p-5 border border-gray-200">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="flex flex-col">
                    ...
                    <select name="tahun" class="w-full h-10 border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all text-sm">
                ...
                <div class="flex flex-col">
                    ...
                    <select name="kabupaten_id" class="w-full h-10 border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all text-sm">
                ...
                <div class="flex flex-col">
                    ...
                    <div class="flex gap-2 h-10">
                        <button type="submit" class="flex-1 h-10 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-lg hover:from-blue-600 hover:to-indigo-700 font-semibold shadow-md transition-all text-sm flex items-center justify-center gap-2">
                            <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"/>
                            </svg>
                            Tampilkan
                        </button>
                        <a href="{{ route('ksa.produksi.bulanan.index') }}" class="h-10 px-4 flex items-center justify-center bg-gray-100 text-gray-600 rounded-lg hover:bg-gray-200 font-semibold text-sm transition-all whitespace-nowrap">
                            Reset
                        </a>
                    </div>
                </div>
    ...
    <div class="bg-gradient-to-r from-blue-50 to-indigo-50 border-l-4 border-blue-500 p-4 mb-6 rounded-r-lg shadow-md">
        <div class="flex items-center gap-3">
            ...
            <p class="text-sm text-blue-800 font-medium">
                ...
                <a href="{{ route('ksa.produksi.index') }}" class="underline font-bold">Sanding Tahunan per Bulan</a> dan
                <a href="{{ route('ksa.produksi.total.index') }}" class="underline font-bold">Sanding Total Tahunan</a>.
                Satuan: <strong>Ton GKG</strong>
            </p>
        </div>
    </div>
            ...
            <table class="min-w-full text-xs border-collapse">
                <thead>
                    <tr class="bg-gradient-to-r from-blue-600 to-indigo-600 text-white">
                        ...
                        <th class="border border-blue-700 px-4 py-2 text-center font-bold" colspan="12">
                            Produksi Padi (Ton GKG) Tahun {{ $tahun }}
                        </th>
                        <th class="border border-blue-700 px-4 py-3 text-center font-bold min-w-[100px]" rowspan="2">Total</th>
                        @if(Auth::user()->canManageData())
                        <th class="border border-blue-700 px-3 py-3 text-center font-bold" rowspan="2">Aksi</th>
                        @endif
                    </tr>
                    <tr class="bg-gradient-to-r from-blue-500 to-indigo-500 text-white text-xs">
                        @foreach($bulanList as $b => $nama)
                        <th class="border border-blue-600 px-2 py-2 text-center font-bold min-w-[64px]">{{ substr($nama,0,3) }}</th>
                        @endforeach
                    </tr>
                </thead>
                <tbody class="bg-white">
                    ...
                    <tr class="hover:bg-blue-50 transition-colors duration-150 {{ $i % 2 == 0 ? 'bg-gray-50' : 'bg-white' }}">
                        ...
                        @foreach($bulanList as $b => $nama)
                        @php $rec = $kabRows->firstWhere('bulan', $b); $val = $rec?->produksi ?? null; @endphp
                        ...
                        <td class="border border-gray-200 px-2 py-2 text-center whitespace-nowrap">
                            ...
                            <div class="flex items-center justify-center gap-1">
                                <a href="{{ route('ksa.produksi.bulanan.edit', ['kabupaten_id' => $kab->id, 'tahun' => $tahun]) }}"
                                   class="inline-flex items-center px-2 py-1 bg-blue-500 text-white rounded hover:bg-blue-600 text-xs font-semibold transition-colors">
                                ...
                                <form method="POST" action="{{ route('ksa.produksi.bulanan.destroy', ['kabupaten_id' => $kab->id]) }}" class="inline"
                                      onsubmit="return confirm('Hapus semua data {{ addslashes($kab->nama_kabupaten) }} tahun {{ $tahun }}?')">
                                    @csrf @method('DELETE')
                ...
                <tfoot>
                    <tr class="bg-gradient-to-r from-blue-700 to-indigo-700 text-white font-bold">
                        <td colspan="2" class="border border-blue-800 px-4 py-3 text-center font-extrabold tracking-wide uppercase">SUMATERA SELATAN</td>
                        @foreach($bulanList as $b => $nama)
                        @php $tot = $totalPerBulan->get($b, 0); @endphp
                        <td class="border border-blue-800 px-2 py-3 text-right font-extrabold">
                            {{ fmtVal($tot) }}
                        </td>
                        @endforeach
                        ...
                        @if(Auth::user()->canManageData())
                        <td class="border border-blue-800 px-2 py-3"></td>
                        @endif
                    </tr>
                </tfoot>
            </table>
    ...
    <p class="mt-3 text-xs text-gray-400">* Satuan: Ton GKG (Gabah Kering Giling) &nbsp;|&nbsp; Tahun: {{ $tahun }}</p>
    ...
    <div class="mt-6 grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div class="bg-gradient-to-br from-cyan-500 to-cyan-700 rounded-xl p-6 text-white shadow-xl">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs mt-1 opacity-75">Jan–Des {{ $tahun }}</p>
                </div>
                <div class="bg-white/20 p-3 rounded-lg"><svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg></div>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-indigo-500 to-indigo-700 rounded-xl p-6 text-white shadow-xl">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm opacity-90">Total Produksi {{ $tahun }}</p>
