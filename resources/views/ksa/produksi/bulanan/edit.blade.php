...
@section('title', 'Edit KSA Produksi Padi')

@section('content')
<div class="max-w-3xl mx-auto px-4 py-8">

    <div class="mb-6">
        <a href="{{ route('ksa.produksi.bulanan.index', ['tahun' => $tahun]) }}"
    ...
    {{-- Info banner --}}
    <div class="mb-4 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-2xl px-5 py-4 flex items-center gap-4 shadow-sm">
        ...
        <div class="flex-1">
            <p class="text-xs text-blue-500 font-semibold uppercase tracking-wide">Sedang mengedit</p>
            <p class="text-sm text-blue-900 font-bold mt-0.5">{{ $kabupaten->nama_kabupaten }} &mdash; Tahun {{ $tahun }}</p>
        </div>
        <div class="text-right">
            <p class="text-xs text-blue-400 font-medium">Bulan terisi</p>
            <p class="text-xl font-extrabold text-blue-700">{{ count($existingData) }} <span class="text-xs font-semibold text-blue-400">/ 12</span></p>
        </div>
    </div>
    ...
    <div class="relative bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
        ...
        <div class="relative bg-gradient-to-br from-blue-500 via-blue-600 to-indigo-700 px-8 py-8 overflow-hidden">
            ...
            <div class="relative flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="mt-0.5 text-blue-100 text-sm font-medium">{{ $kabupaten->nama_kabupaten }} &mdash; Tahun {{ $tahun }}</p>
                </div>
            </div>
            <div class="relative mt-5 flex items-center gap-2 text-xs">
                ...
                <svg class="w-3 h-3 text-blue-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                <span class="bg-white text-blue-700 px-3 py-1 rounded-full font-bold shadow">Edit</span>
            </div>
        </div>

        {{-- Tahun sebagai hidden input (tidak bisa diubah di edit) --}}
        <form method="POST" action="{{ route('ksa.produksi.bulanan.update', ['kabupaten_id' => $kabupaten->id, 'tahun' => $tahun]) }}" class="p-8 space-y-7" id="formEdit">
            @csrf @method('PUT')
            ...
            <div>
                <div class="flex items-center justify-between mb-3">
                    ...
                    <div id="counterBadge" class="text-xs font-bold px-3 py-1 rounded-full bg-blue-100 text-blue-700 flex items-center gap-1">
                ...
                <div class="grid grid-cols-3 gap-3" id="bulanGrid">
                    ...
                    @php
                        ...
                        $val = old('produksi.'.$b, $existing?->produksi ?? '');
                        $hasPrev = $existing !== null;
                    @endphp
                    <div class="bulan-item rounded-xl border overflow-hidden transition-all duration-200 hover:shadow-sm
                        {{ $val !== '' ? 'border-blue-400 bg-blue-50' : 'border-gray-200 bg-gray-50 hover:border-blue-300 hover:bg-blue-50/30' }}">
                        <div class="flex items-center justify-between px-3 pt-3 pb-1">
                            ...
                            @if($hasPrev)
                            <span class="text-xs text-blue-400 font-semibold bg-blue-100 px-1.5 py-0.5 rounded-md">Ada data</span>
                            @endif
                        </div>
        ...
        <div class="px-8 pb-8">
            <div class="rounded-xl border border-red-100 bg-red-50/50 p-5">
                ...
                <p class="text-xs text-red-400 mb-4">Hapus semua data produksi <strong>{{ $kabupaten->nama_kabupaten }}</strong> tahun <strong>{{ $tahun }}</strong>. Tidak dapat dibatalkan.</p>
                <form method="POST" action="{{ route('ksa.produksi.bulanan.destroy', ['kabupaten_id' => $kabupaten->id]) }}"
                      onsubmit="return confirm('Yakin hapus SEMUA data {{ addslashes($kabupaten->nama_kabupaten) }} tahun {{ $tahun }}?\n\nTidak dapat dibatalkan.')">
                    @csrf @method('DELETE')
                    <input type="hidden" name="tahun" value="{{ $tahun }}">
