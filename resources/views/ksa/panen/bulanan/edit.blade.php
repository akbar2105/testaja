...
@section('title', 'Edit KSA Luas Panen')
...
<div class="max-w-3xl mx-auto px-4 py-8">
    ...
    {{-- Back --}}
    <div class="mb-6">
        <a href="{{ route('ksa.panen.bulanan.index', ['tahun' => $tahun]) }}"
           class="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-violet-600 transition-colors font-semibold">
            ...
            Kembali ke Sanding Bulanan {{ $tahun }}
        </a>
    </div>

    {{-- Info banner --}}
    <div class="mb-4 bg-gradient-to-r from-violet-50 to-indigo-50 border border-violet-200 rounded-2xl px-5 py-4 flex items-center gap-4 shadow-sm">
        ...
        <div class="flex-1">
            <p class="text-xs text-violet-500 font-semibold uppercase tracking-wide">Sedang mengedit</p>
            <p class="text-sm text-violet-900 font-bold mt-0.5">
                {{ $kabupaten->nama_kabupaten }} &mdash; Tahun {{ $tahun }}
            </p>
        </div>
        <div class="text-right">
            <p class="text-xs text-violet-400 font-medium">Bulan terisi</p>
            <p class="text-xl font-extrabold text-violet-700">{{ count($existingData) }} <span class="text-xs font-semibold text-violet-400">/ 12</span></p>
        </div>
    </div>
    ...
    <div class="relative bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
        ...
        <div class="relative bg-gradient-to-br from-violet-500 via-violet-600 to-indigo-700 px-8 py-8 overflow-hidden">
            ...
            <div class="relative flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="mt-0.5 text-violet-200 text-sm font-medium">
                        {{ $kabupaten->nama_kabupaten }} &mdash; Tahun {{ $tahun }}
                    </p>
                </div>
            </div>
            ...
            <div class="relative mt-5 flex items-center gap-2 text-xs">
                ...
                <svg class="w-3 h-3 text-violet-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                <span class="bg-white text-violet-700 px-3 py-1 rounded-full font-bold shadow">Edit</span>
            </div>
        </div>

        <form method="POST" action="{{ route('ksa.panen.bulanan.update', ['kabupaten_id' => $kabupaten->id, 'tahun' => $tahun]) }}" class="p-8 space-y-7" id="formEdit">
            @csrf
            @method('PUT')
            {{-- Tahun dikirim sebagai hidden — tidak bisa diubah dari halaman edit --}}
            ...
            <div>
                <div class="flex items-center justify-between mb-3">
                    ...
                    <div id="counterBadge" class="text-xs font-bold px-3 py-1 rounded-full bg-violet-100 text-violet-700 flex items-center gap-1">
                ...
                <div class="grid grid-cols-3 gap-3" id="bulanGrid">
                    ...
                    @php
                        ...
                        $val = old('luas_panen.'.$b, $existing?->luas_panen ?? '');
                        $hasPrev = $existing !== null;
                    @endphp
                    <div class="bulan-item rounded-xl border overflow-hidden transition-all duration-200 hover:shadow-sm
                        {{ $val !== '' ? 'border-violet-400 bg-violet-50' : 'border-gray-200 bg-gray-50 hover:border-violet-300 hover:bg-violet-50/30' }}">
                        <div class="flex items-center justify-between px-3 pt-3 pb-1">
                            ...
                            @if($hasPrev)
                            <span class="text-xs text-violet-400 font-semibold bg-violet-100 px-1.5 py-0.5 rounded-md">Ada data</span>
                            @endif
                        </div>
        ...
        <div class="px-8 pb-8">
            <div class="rounded-xl border border-red-100 bg-red-50/50 p-5">
                ...
                <p class="text-xs text-red-400 mb-4">Hapus semua data panen <strong>{{ $kabupaten->nama_kabupaten }}</strong> tahun <strong>{{ $tahun }}</strong>. Tindakan ini tidak dapat dibatalkan.</p>
                <form method="POST" action="{{ route('ksa.panen.bulanan.destroy', ['kabupaten_id' => $kabupaten->id, 'tahun' => $tahun]) }}"
                      onsubmit="return confirm('Yakin hapus SEMUA data panen {{ addslashes($kabupaten->nama_kabupaten) }} tahun {{ $tahun }}?\n\nTindakan ini tidak dapat dibatalkan.')">
                    @csrf @method('DELETE')
                    <button type="submit"
                            class="inline-flex items-center gap-2 px-4 py-2 bg-white text-red-600 border border-red-200 rounded-lg hover:bg-red-600 hover:text-white hover:border-red-600 text-sm font-semibold transition-all duration-200 shadow-sm">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                        </svg>
                        Hapus Semua Data Tahun {{ $tahun }}
                    </button>
                </form>
            </div>
        </div>
