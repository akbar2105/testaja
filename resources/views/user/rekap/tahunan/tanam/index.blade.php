...
@section('title', 'Rekap Tahunan Luas Tanam')
...
<div class="max-w-full mx-auto px-4 py-6">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex gap-2 flex-wrap justify-end">
            <a href="{{ route('user.rekap.tahunan.tanam.export', request()->query()) }}"
        ...
        <div class="bg-white rounded-xl shadow-md p-6 border border-gray-200">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div class="flex items-end gap-2">
                    ...
                    <a href="{{ route('user.rekap.tahunan.tanam.index') }}" class="px-4 py-2.5 bg-gray-100 text-gray-600 rounded-lg hover:bg-gray-200 font-semibold text-sm transition-all">Reset</a>
                </div>
            </div>
        </div>
    </form>
    ...
    <div class="bg-gradient-to-r from-green-50 to-emerald-50 border-l-4 border-green-500 p-4 mb-6 rounded-r-lg shadow-md">
        <div class="flex items-center">
            ...
            <p class="ml-3 text-sm text-green-800 font-medium">
                <strong>Informasi:</strong> Data rekap tahunan dihitung dari total luas tanam Januari s.d. Desember per kabupaten &bull;
            ...
            <table class="min-w-full text-xs border-collapse">
                <thead>
                    ...
                    <tr class="bg-gradient-to-r from-green-200 to-green-300 text-gray-800 text-center font-bold">
                        ...
                        <th class="border border-gray-300 px-4 py-3 text-center font-bold" colspan="{{ $tahunDipilih->count() }}">Total LTT Januari – Desember</th>
                ...
                <tfoot>
                    <tr class="bg-gradient-to-r from-green-500 to-green-600 text-white font-bold">
                        <td class="border border-teal-400 px-3 py-3 text-center" colspan="2">
                            <span class="text-xs uppercase tracking-wide font-extrabold">SUMATERA SELATAN</span>
                        </td>
                        @foreach($tahunDipilih as $tahun)
                            @php $tPT = $totalPerTahun->get($tahun, 0); @endphp
                            <td class="border border-teal-400 px-3 py-3 text-right">{{ $tPT == 0 ? '0' : (floor($tPT) == $tPT ? number_format($tPT, 0, ',', '.') : number_format($tPT, 2, ',', '.')) }}</td>
                        @endforeach
                        <td class="border border-teal-400 px-4 py-3 text-right text-white font-extrabold text-sm">
    ...
    <div class="mt-6 grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div class="bg-gradient-to-br from-teal-500 to-teal-700 rounded-xl p-6 text-white shadow-xl">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm opacity-90">Luas Tanam Tertinggi</p>
