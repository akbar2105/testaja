...
<div class="max-w-full mx-auto px-4 py-6">
    ...
    {{-- HEADER --}}
    <div class="mb-6">
        <div class="bg-gradient-to-br from-orange-400 via-orange-500 to-amber-600 rounded-2xl shadow-2xl p-7 text-white">
            <div class="flex items-center justify-between flex-wrap gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-2xl md:text-3xl font-black mb-1">Grafik LTT — Luas Tanam Padi</h1>
                        <p class="text-orange-100 text-sm font-medium">Harian • Bulanan • Tahunan • Provinsi Sumatera Selatan</p>
                    </div>
                </div>
                <div class="bg-white/15 backdrop-blur-md rounded-xl px-4 py-3 border border-white/20 text-center">
                    <p class="text-orange-100 text-xs font-semibold uppercase">Kabupaten</p>
            ...
            @foreach($tabs as $key => $t)
                <a href="{{ request()->fullUrlWithQuery(['tab' => $key]) }}"
                   class="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-bold transition-all duration-200
                          {{ $tab === $key ? 'bg-gradient-to-r from-orange-500 to-amber-500 text-white shadow-md' : 'text-gray-600 hover:bg-orange-50 hover:text-orange-700' }}">
    ...
    <div>
        <form method="GET" class="mb-6">
            ...
            <div class="bg-white rounded-2xl shadow-lg p-5 border border-gray-200 flex flex-wrap items-end gap-4">
                <div>
                    ...
                    <input type="number" name="tahun_harian" value="{{ $tahunHarian }}" min="2000" max="2099" class="px-4 py-2.5 border-2 border-gray-200 rounded-xl focus:border-orange-400 font-semibold text-sm w-28">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-600 mb-2 uppercase tracking-wide">Bulan</label>
                    <select name="bulan_harian" class="px-4 py-2.5 border-2 border-gray-200 rounded-xl focus:border-orange-400 font-semibold text-sm bg-white">
                ...
                <button type="submit" class="px-6 py-2.5 bg-gradient-to-r from-orange-500 to-amber-500 text-white rounded-xl font-bold text-sm hover:from-orange-600 hover:to-amber-600 shadow transition-all">Tampilkan</button>
            </div>
        </form>

        <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden mb-6">
            <div class="bg-gradient-to-r from-orange-500 to-amber-500 px-6 py-4 flex items-center justify-between gap-3">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-lg font-black text-white">Luas Tanam Harian — {{ $bnama[$bulanHarian] }} {{ $tahunHarian }}</h2>
                </div>
                <div class="flex items-center gap-2">
                    ...
                    <button onclick="downloadPng('harChart','ltt-tanam-harian-{{ $bulanHarian }}-{{ $tahunHarian }}')" class="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold bg-white/20 text-white hover:bg-white/40 transition-all border border-white/30">
                    ...
                    <a id="pdf-btn-harian" href="{{ route('user.grafik.ltt.tanam.pdf', ['tab'=>'harian','tahun_harian'=>$tahunHarian,'bulan_harian'=>$bulanHarian,'chart_mode'=>'grouped']) }}" class="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold bg-white/20 text-white hover:bg-white/40 transition-all border border-white/30">
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
            <div class="bg-gradient-to-r from-orange-500 to-amber-500 px-6 py-4">
                <h2 class="text-lg font-black text-white">Rekapitulasi Harian Luas Tanam (Ha) — {{ $bnama[$bulanHarian] }} {{ $tahunHarian }}</h2>
            </div>
            <div class="overflow-x-auto">
                <table class="min-w-full text-xs border-collapse">
                    <thead>
                        <tr class="bg-orange-50">
                            <th class="border border-orange-200 px-3 py-3 text-left font-black text-orange-900 sticky left-0 bg-orange-50 min-w-[150px]">Kabupaten/Kota</th>
                            @for($d = 1; $d <= $jumlahHariBulan; $d++)
                                <th class="border border-orange-200 px-2 py-3 text-center font-black text-orange-900 min-w-[38px]">{{ $d }}</th>
                            @endfor
                            <th class="border border-orange-200 px-3 py-3 text-center font-black text-orange-900 bg-orange-100 min-w-[80px]">Total</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($dataHarian as $index => $row)
                            <tr class="{{ $index % 2 == 0 ? 'bg-white' : 'bg-orange-50/30' }} hover:bg-orange-50 transition-colors">
                                <td class="border border-gray-200 px-3 py-2 font-semibold text-gray-900 sticky left-0 {{ $index % 2 == 0 ? 'bg-white' : 'bg-orange-50/30' }}">
                                ...
                                @endfor
                                <td class="border border-orange-200 px-3 py-2 text-right font-bold bg-orange-50 text-orange-800">{{ fmtVal($row['total']) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                    <tfoot>
                        <tr class="bg-gradient-to-r from-orange-500 to-amber-500 text-white">
                            <td class="border border-orange-700 px-3 py-3 font-black sticky left-0 bg-orange-500">TOTAL</td>
                            @for($d = 1; $d <= $jumlahHariBulan; $d++)
                                @php $colTotal = array_sum(array_column($dataHarian, 'tgl_'.$d)); @endphp
                                <td class="border border-orange-700 px-2 py-3 text-right font-bold">{{ fmtVal($colTotal) }}</td>
                            @endfor
                            <td class="border border-orange-700 px-3 py-3 text-right font-black">{{ fmtVal(array_sum($totalHarian)) }}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
    ...
    <div>
        <form method="GET" class="mb-6">
            ...
            <div class="bg-white rounded-2xl shadow-lg p-5 border border-gray-200 flex flex-wrap items-end gap-4">
                <div>
                    ...
                    <select name="tahun_bulanan" class="px-4 py-2.5 border-2 border-gray-200 rounded-xl focus:border-orange-400 font-semibold text-sm bg-white min-w-[120px]">
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden mb-6">
            <div class="bg-gradient-to-r from-orange-500 to-amber-500 px-6 py-4 flex items-center justify-between gap-3">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-lg font-black text-white">Luas Tanam per Bulan — Tahun {{ $tahunBulanan }}</h2>
                </div>
                <div class="flex items-center gap-2">
                    ...
                    <button onclick="downloadPng('bulChart','ltt-tanam-bulanan-{{ $tahunBulanan }}')" class="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold bg-white/20 text-white hover:bg-white/40 transition-all border border-white/30">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                        PNG
                    </button>
                    <a id="pdf-btn-bulanan" href="{{ route('user.grafik.ltt.tanam.pdf', ['tab'=>'bulanan','tahun_bulanan'=>$tahunBulanan,'chart_mode'=>'grouped']) }}" class="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold bg-white/20 text-white hover:bg-white/40 transition-all border border-white/30">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"/></svg>
                        PDF
                    </a>
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
            <div class="bg-gradient-to-r from-orange-500 to-amber-500 px-6 py-4">
                <h2 class="text-lg font-black text-white">Tabel Luas Tanam Bulanan (Ha) — Tahun {{ $tahunBulanan }}</h2>
            </div>
            <div class="overflow-x-auto">
                <table class="min-w-full text-xs border-collapse">
                    <thead>
                        <tr class="bg-orange-50">
                            <th class="border border-orange-200 px-4 py-3 text-left font-black text-orange-900 sticky left-0 bg-orange-50">Kabupaten/Kota</th>
                            @foreach(['Jan','Feb','Mar','Apr','Mei','Jun','Jul','Agt','Sep','Okt','Nov','Des'] as $bl)
                                <th class="border border-orange-200 px-3 py-3 text-center font-black text-orange-900">{{ $bl }}</th>
                            @endforeach
                            <th class="border border-orange-200 px-3 py-3 text-center font-black text-orange-900 bg-orange-100">Total</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($dataBulanan as $index => $row)
                            ...
                            <tr class="{{ $index % 2 == 0 ? 'bg-white' : 'bg-orange-50/30' }} hover:bg-orange-50 transition-colors">
                                <td class="border border-gray-200 px-4 py-2.5 font-semibold text-gray-900 sticky left-0 {{ $index % 2 == 0 ? 'bg-white' : 'bg-orange-50/30' }}">
                                    <span class="inline-block w-2.5 h-2.5 rounded-sm mr-1.5" style="background:{{ $chartColors[$index % $chartColorsCount] }}"></span>
                                    {{ $row['kabupaten'] }}
                                </td>
                                ...
                                @endforeach
                                <td class="border border-orange-200 px-3 py-2.5 text-right font-bold bg-orange-50 text-orange-800">{{ fmtVal($rowTotal) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                    <tfoot>
                        <tr class="bg-gradient-to-r from-orange-500 to-amber-500 text-white">
                            <td class="border border-orange-700 px-4 py-3 font-black">TOTAL</td>
                            @foreach($totalBulanan as $t)
                                <td class="border border-orange-700 px-3 py-3 text-right font-bold">{{ fmtVal($t) }}</td>
                            @endforeach
                            <td class="border border-orange-700 px-3 py-3 text-right font-black">{{ fmtVal(array_sum($totalBulanan)) }}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
        ...
        <form method="GET" class="mb-6">
            ...
            <div class="bg-white rounded-2xl shadow-lg p-5 border border-gray-200 flex flex-wrap items-end gap-4">
                <div>
                    ...
                    <select name="tahun_awal_tahunan" class="px-4 py-2.5 border-2 border-gray-200 rounded-xl focus:border-orange-400 font-semibold text-sm bg-white min-w-[120px]">
                ...
                <div>
                    ...
                    <select name="tahun_akhir_tahunan" class="px-4 py-2.5 border-2 border-gray-200 rounded-xl focus:border-orange-400 font-semibold text-sm bg-white min-w-[120px]">
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden mb-6">
            <div class="bg-gradient-to-r from-orange-500 to-amber-500 px-6 py-4 flex items-center justify-between gap-3">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-lg font-black text-white">Total Luas Tanam per Tahun — {{ $tahunAwalT }}-{{ $tahunAkhirT }}</h2>
                </div>
                <div class="flex items-center gap-2">
                    <button onclick="downloadPng('tahChart','ltt-tanam-tahunan-{{ $tahunAwalT }}-{{ $tahunAkhirT }}')" class="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold bg-white/20 text-white hover:bg-white/40 transition-all border border-white/30">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                        PNG
                    </button>
                    <a href="{{ route('user.grafik.ltt.tanam.pdf', ['tab'=>'tahunan','tahun_awal_tahunan'=>$tahunAwalT,'tahun_akhir_tahunan'=>$tahunAkhirT,'chart_mode'=>'grouped']) }}" class="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold bg-white/20 text-white hover:bg-white/40 transition-all border border-white/30">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"/></svg>
                        PDF
                    </a>
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-xs border-collapse">
                    <thead>
                        <tr class="bg-orange-50">
                            <th class="border border-orange-200 px-4 py-3 text-left font-black text-orange-900">Kabupaten/Kota</th>
                            @foreach($tahunRangeT as $idx => $t)
                    ...
                    <tfoot>
                        <tr class="bg-gradient-to-r from-orange-500 to-amber-500 text-white">
                            <td class="border border-orange-700 px-4 py-3 font-black">TOTAL</td>
                            @foreach($tahunRangeT as $t)
                                <td class="border border-orange-700 px-3 py-3 text-right font-bold">{{ fmtVal($totalTahunan[$t] ?? 0) }}</td>
                            @endforeach
                        </tr>
                    </tfoot>
                </table>
