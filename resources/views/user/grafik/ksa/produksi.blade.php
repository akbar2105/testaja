...
<div class="max-w-full mx-auto px-4 py-6">
    ...
    {{-- HEADER --}}
    <div class="mb-6">
        <div class="bg-gradient-to-br from-blue-500 via-blue-600 to-indigo-700 rounded-2xl shadow-2xl p-7 text-white">
            <div class="flex items-center justify-between flex-wrap gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-2xl md:text-3xl font-black mb-1">Grafik KSA — Produksi</h1>
                        <p class="text-blue-100 text-sm font-medium">Bulanan • Total Tahunan • Provinsi Sumatera Selatan</p>
                    </div>
                </div>
                <div class="bg-white/15 backdrop-blur-md rounded-xl px-4 py-3 border border-white/20 text-center">
                    <p class="text-blue-100 text-xs font-semibold uppercase">Kabupaten</p>
    ...
    <div class="mb-6">
        <div class="bg-white rounded-2xl shadow-lg border border-gray-200 p-1.5 flex gap-1">
            @php $tabs = [
                'bulanan' => ['label'=>'Sanding Bulanan','icon'=>'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z'],
            ...
            @foreach($tabs as $key => $t)
                <a href="{{ request()->fullUrlWithQuery(['tab' => $key]) }}"
                   class="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-bold transition-all duration-200
                          {{ $tab === $key ? 'bg-gradient-to-r from-blue-500 to-indigo-600 text-white shadow-md' : 'text-gray-600 hover:bg-blue-50 hover:text-blue-700' }}">
    ...
    <div>
        <form method="GET" class="mb-6">
            ...
            <div class="bg-white rounded-2xl shadow-lg p-5 border border-gray-200 flex flex-wrap items-end gap-4">
                <div>
                    ...
                    <select name="tahun_bulanan" class="px-4 py-2.5 border-2 border-gray-200 rounded-xl focus:border-blue-400 font-semibold text-sm bg-white min-w-[120px]">
                        @foreach($tahunTersedia as $t)
                            <option value="{{ $t }}" {{ $tahunBulanan == $t ? 'selected' : '' }}>{{ $t }}</option>
                        @endforeach
                    </select>
                </div>
                <button type="submit" class="px-6 py-2.5 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-xl font-bold text-sm hover:from-blue-600 hover:to-indigo-700 shadow transition-all">Tampilkan</button>
            </div>
        </form>

        <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden mb-6">
            <div class="bg-gradient-to-r from-blue-500 to-indigo-600 px-6 py-4 flex items-center justify-between gap-3">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-lg font-black text-white">KSA Produksi per Bulan — Tahun {{ $tahunBulanan }}</h2>
                </div>
                <div class="flex items-center gap-2">
                    ...
                    <button onclick="downloadPng('bulananChart','ksa-produksi-bulanan-{{ $tahunBulanan }}')" class="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold bg-white/20 text-white hover:bg-white/40 transition-all border border-white/30">
                    ...
                    <a id="pdf-btn-produksi-bulanan" href="{{ route('user.grafik.ksa.produksi.pdf', ['tab'=>'bulanan','tahun_bulanan'=>$tahunBulanan,'chart_mode'=>'stacked']) }}" class="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold bg-white/20 text-white hover:bg-white/40 transition-all border border-white/30">
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
            <div class="bg-gradient-to-r from-blue-500 to-indigo-600 px-6 py-4">
                <h2 class="text-lg font-black text-white">Tabel KSA Produksi Bulanan (Ton) — Tahun {{ $tahunBulanan }}</h2>
            </div>
            <div class="overflow-x-auto">
                <table class="min-w-full text-xs border-collapse">
                    <thead>
                        <tr class="bg-blue-50">
                            <th class="border border-blue-200 px-4 py-3 text-left font-black text-blue-900 sticky left-0 bg-blue-50">Kabupaten/Kota</th>
                            @foreach(['Jan','Feb','Mar','Apr','Mei','Jun','Jul','Agt','Sep','Okt','Nov','Des'] as $bl)
                                <th class="border border-blue-200 px-3 py-3 text-center font-black text-blue-900">{{ $bl }}</th>
                            @endforeach
                            <th class="border border-blue-200 px-3 py-3 text-center font-black text-blue-900 bg-blue-100">Total</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($dataBulanan as $index => $row)
                            @php $rowTotal = array_sum($row['data']); @endphp
                            <tr class="{{ $index % 2 == 0 ? 'bg-white' : 'bg-blue-50/20' }} hover:bg-blue-50 transition-colors">
                                <td class="border border-gray-200 px-4 py-2.5 font-semibold text-gray-900 sticky left-0 {{ $index % 2 == 0 ? 'bg-white' : 'bg-blue-50/20' }}">
                                ...
                                @endforeach
                                <td class="border border-blue-200 px-3 py-2.5 text-right font-bold bg-blue-50 text-blue-800">{{ fmtVal($rowTotal) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                    <tfoot>
                        <tr class="bg-gradient-to-r from-blue-500 to-indigo-600 text-white">
                            <td class="border border-blue-700 px-4 py-3 font-black">TOTAL</td>
                            @foreach($totalBulanan as $t)
                                <td class="border border-blue-700 px-3 py-3 text-right font-bold">{{ fmtVal($t) }}</td>
                            @endforeach
                            <td class="border border-blue-700 px-3 py-3 text-right font-black">{{ fmtVal(array_sum($totalBulanan)) }}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
        ...
        <form method="GET" class="mb-6">
            ...
            <div class="bg-white rounded-2xl shadow-lg p-5 border border-gray-200 flex flex-wrap items-end gap-4">
                <div>
                    ...
                    <select name="tahun_awal_tahunan" class="px-4 py-2.5 border-2 border-gray-200 rounded-xl focus:border-blue-400 font-semibold text-sm bg-white min-w-[120px]">
                ...
                <div>
                    ...
                    <select name="tahun_akhir_tahunan" class="px-4 py-2.5 border-2 border-gray-200 rounded-xl focus:border-blue-400 font-semibold text-sm bg-white min-w-[120px]">
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden mb-6">
            <div class="bg-gradient-to-r from-blue-500 to-indigo-600 px-6 py-4 flex items-center justify-between gap-3">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-lg font-black text-white">Total KSA Produksi per Tahun — {{ $tahunAwalT }}-{{ $tahunAkhirT }}</h2>
                </div>
                <div class="flex items-center gap-2">
                    <button onclick="downloadPng('tahunanChart','ksa-produksi-tahunan-{{ $tahunAwalT }}-{{ $tahunAkhirT }}')" class="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold bg-white/20 text-white hover:bg-white/40 transition-all border border-white/30">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                        PNG
                    </button>
                    <a href="{{ route('user.grafik.ksa.produksi.pdf', ['tab'=>'tahunan','tahun_awal_tahunan'=>$tahunAwalT,'tahun_akhir_tahunan'=>$tahunAkhirT,'chart_mode'=>'stacked']) }}" class="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold bg-white/20 text-white hover:bg-white/40 transition-all border border-white/30">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"/></svg>
                        PDF
                    </a>
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-xs border-collapse">
                    <thead>
                        <tr class="bg-blue-50">
                            <th class="border border-blue-200 px-4 py-3 text-left font-black text-blue-900">Kabupaten/Kota</th>
                            @foreach($tahunRangeT as $idx => $t)
                    ...
                    <tfoot>
                        <tr class="bg-gradient-to-r from-blue-500 to-indigo-600 text-white">
                            <td class="border border-blue-700 px-4 py-3 font-black">TOTAL</td>
                            @foreach($tahunRangeT as $t)
                                <td class="border border-blue-700 px-3 py-3 text-right font-bold">{{ fmtVal($totalTahunan[$t] ?? 0) }}</td>
                            @endforeach
                        </tr>
                    </tfoot>
                </table>
        ...
        options:{
            ...
            plugins:{
                ...
                tooltip:{backgroundColor:'rgba(0,0,0,0.85)', callbacks:{
                    label: ctx=>' '+ctx.dataset.label+': '+fmtFull(ctx.parsed.y)+' Ton',
                    afterBody: ctx=>[' ─────────────',' Total: '+fmtFull(bulTotal[ctx[0].dataIndex])+' Ton']
                }}
            },
            scales:{
...
function setMode(m) {
    ...
    syncPdfBtn('pdf-btn-produksi-bulanan', m);
    makeBulChart(m);
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    new Chart(document.getElementById('tahunanChart').getContext('2d'), {
        ...
        options:{
            responsive:true, maintainAspectRatio:false,
            layout:{padding:{top:20}},
            plugins:{legend:{display:false},tooltip:{backgroundColor:'rgba(0,0,0,0.85)',callbacks:{label:ctx=>' '+ctx.dataset.label+': '+fmtFull(ctx.parsed.y)+' Ton'}}},
            scales:{
