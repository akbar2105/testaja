        ...
        <div class="bg-gradient-to-br from-violet-500 via-purple-600 to-indigo-700 rounded-2xl shadow-2xl p-7 text-white">
            <div class="flex items-center justify-between flex-wrap gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-2xl md:text-3xl font-black mb-1">Grafik KSA — Luas Panen</h1>
                        <p class="text-violet-100 text-sm font-medium">Bulanan • Total Tahunan • Provinsi Sumatera Selatan</p>
                    </div>
                </div>
            ...
            @foreach($tabs as $key => $t)
                <a href="{{ request()->fullUrlWithQuery(['tab' => $key]) }}"
                   class="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-bold transition-all duration-200
                          {{ $tab === $key ? 'bg-gradient-to-r from-violet-500 to-indigo-600 text-white shadow-md' : 'text-gray-600 hover:bg-violet-50 hover:text-violet-700' }}">
    ...
    <div>
        <form method="GET" class="mb-6">
            ...
            <div class="bg-white rounded-2xl shadow-lg p-5 border border-gray-200 flex flex-wrap items-end gap-4">
                ...
                <button type="submit" class="px-6 py-2.5 bg-gradient-to-r from-violet-500 to-indigo-600 text-white rounded-xl font-bold text-sm hover:from-violet-600 hover:to-indigo-700 shadow transition-all">Tampilkan</button>
            </div>
        </form>

        <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden mb-6">
            <div class="bg-gradient-to-r from-violet-500 to-indigo-600 px-6 py-4 flex items-center justify-between gap-3">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-lg font-black text-white">KSA Luas Panen per Bulan — Tahun {{ $tahunBulanan }}</h2>
                </div>
                <div class="flex items-center gap-2">
                    ...
                    <button onclick="downloadPng('bulananChart','ksa-panen-bulanan-{{ $tahunBulanan }}')" class="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold bg-white/20 text-white hover:bg-white/40 transition-all border border-white/30">
                    ...
                    <a id="pdf-btn-bulanan" href="{{ route('user.grafik.ksa.panen.pdf', ['tab'=>'bulanan','tahun_bulanan'=>$tahunBulanan,'chart_mode'=>'stacked']) }}" class="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold bg-white/20 text-white hover:bg-white/40 transition-all border border-white/30">
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
            <div class="bg-gradient-to-r from-violet-500 to-indigo-600 px-6 py-4">
                <h2 class="text-lg font-black text-white">Tabel KSA Luas Panen Bulanan (Ha) — Tahun {{ $tahunBulanan }}</h2>
            </div>
                ...
                <table class="min-w-full text-xs border-collapse">
                    ...
                    <tbody>
                        @foreach($dataBulanan as $index => $row)
                            ...
                            <tr class="{{ $index % 2 == 0 ? 'bg-white' : 'bg-violet-50/20' }} hover:bg-violet-50 transition-colors">
                                <td class="border border-gray-200 px-4 py-2.5 font-semibold text-gray-900 sticky left-0 {{ $index % 2 == 0 ? 'bg-white' : 'bg-violet-50/20' }}">
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden mb-6">
            <div class="bg-gradient-to-r from-violet-500 to-indigo-600 px-6 py-4 flex items-center justify-between gap-3">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-lg font-black text-white">Total KSA Luas Panen per Tahun — {{ $tahunAwalT }}-{{ $tahunAkhirT }}</h2>
                </div>
                <div class="flex items-center gap-2">
                    <button onclick="downloadPng('tahunanChart','ksa-panen-tahunan-{{ $tahunAwalT }}-{{ $tahunAkhirT }}')" class="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold bg-white/20 text-white hover:bg-white/40 transition-all border border-white/30">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                        PNG
                    </button>
                    <a href="{{ route('user.grafik.ksa.panen.pdf', ['tab'=>'tahunan','tahun_awal_tahunan'=>$tahunAwalT,'tahun_akhir_tahunan'=>$tahunAkhirT,'chart_mode'=>'stacked']) }}" class="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold bg-white/20 text-white hover:bg-white/40 transition-all border border-white/30">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"/></svg>
                        PDF
                    </a>
                </div>
