...
@section('title', 'KSA Sanding Total Tahunan Luas Tanam')
...
<div class="max-w-full mx-auto px-4 py-6">
    ...
    <div class="mb-6 flex items-center justify-between flex-wrap gap-4">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 flex items-center gap-3">
                ...
                KSA LTT Padi Sanding Total Tahunan
            </h1>
            <p class="mt-2 text-sm text-gray-600">KSA &mdash; Total LTT per kabupaten per tahun (Okt–Sep) &bull; Satuan: Hektar (Ha)</p>
        </div>
        <div class="flex gap-2 flex-wrap">
            <a href="{{ route('user.ksa.tanam.total.export', ['tahun_awal' => $tahunAwal, 'tahun_akhir' => $tahunAkhir]) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div class="flex flex-col">
                    ...
                    <div class="flex gap-2 h-10">
                        ...
                        <a href="{{ route('user.ksa.tanam.total.index') }}" class="h-10 px-4 flex items-center justify-center bg-gray-100 text-gray-600 rounded-lg hover:bg-gray-200 font-semibold text-sm transition-all whitespace-nowrap">
                            Reset
                        </a>
                    </div>
                </div>
    ...
    {{-- ── INFO ── --}}
    <div class="bg-gradient-to-r from-orange-50 to-amber-50 border-l-4 border-orange-500 p-4 mb-6 rounded-r-lg shadow-sm">
        <div class="flex items-center gap-3">
            <svg class="h-5 w-5 text-orange-600 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd"
                    d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z"
                    clip-rule="evenodd" />
            </svg>
            <p class="text-sm text-orange-800">
                <strong>Sanding Total Tahunan</strong> &mdash; Total LTT <strong>Oktober–September</strong> per
                kabupaten, dihitung otomatis (SUM) dari
                <a href="{{ route('user.ksa.tanam.bulanan.index') }}" class="underline font-bold">Sanding Bulanan</a>.
                Satuan: <strong>Ha</strong>
            </p>
        </div>
    </div>
            ...
            <table class="min-w-full text-xs border-collapse">
                <thead>
                    <tr class="bg-gradient-to-r from-orange-500 to-orange-600 text-white">
                        ...
                        <th class="border border-orange-600 px-4 py-2 text-center font-bold" colspan="{{ $tahunAkhir - $tahunAwal + 1 }}">Total LTT Padi</th>
                        <th class="border border-orange-600 px-3 py-3 text-center font-bold min-w-[110px]" rowspan="2">Total</th>
                    </tr>
                    <tr class="bg-gradient-to-r from-orange-400 to-orange-500 text-white text-xs">
                        @for($y = $tahunAwal; $y <= $tahunAkhir; $y++)
                        <th class="border border-orange-500 px-3 py-2 text-center font-bold min-w-[90px]">Okt {{ $y }}</th>
                        @endfor
                    </tr>
                </thead>
                <tbody class="bg-white">
                    ...
                    <tr class="hover:bg-orange-50 transition-colors duration-150 {{ $i % 2 == 0 ? 'bg-gray-50' : 'bg-white' }}">
                        ...
                        @endfor
                        <td class="border border-orange-200 px-3 py-2.5 text-right font-bold {{ $kabTotal > 0 ? 'text-orange-700 bg-orange-50' : 'text-gray-300' }}">
                            {{ ($kabTotal ?? 0) == 0 ? '0' : (floor($kabTotal) == $kabTotal ? number_format($kabTotal, 0, ',', '.') : number_format($kabTotal, 2, ',', '.')) }}
                        </td>
                    </tr>
                    @empty
                    <tr><td colspan="{{ $tahunAkhir - $tahunAwal + 4 }}" class="py-10 text-center text-gray-400 font-semibold">Belum ada data kabupaten</td></tr>
                    @endforelse
                </tbody>
                <tfoot>
                    <tr class="bg-gradient-to-r from-orange-700 to-orange-800 text-white font-bold">
                        ...
                        @endfor
                        @php $grandTotalFoot = $totalPerTahun->sum(); @endphp
                        <td class="border border-orange-800 px-3 py-3 text-right font-extrabold bg-orange-900">
                            {{ ($grandTotalFoot ?? 0) == 0 ? '0' : (floor($grandTotalFoot) == $grandTotalFoot ? number_format($grandTotalFoot, 0, ',', '.') : number_format($grandTotalFoot, 2, ',', '.')) }}
                        </td>
                    </tr>
                </tfoot>
            </table>
    ...
    <p class="mt-3 text-xs text-gray-400">* Satuan: Hektar (Ha) &nbsp;|&nbsp; Nilai = SUM LTT Oktober–September tiap tahun</p>
    ...
    <div class="mt-6 grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div class="bg-gradient-to-br from-amber-500 to-amber-700 rounded-xl p-6 text-white shadow-xl">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm opacity-90">Total LTT {{ $tahunAkhir }}</p>
