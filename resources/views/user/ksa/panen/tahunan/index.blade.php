...
<div class="max-w-full mx-auto px-4 py-6">
    ...
    <div class="mb-6 flex items-center justify-between flex-wrap gap-4">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 flex items-center gap-3">
                ...
                Luas Panen Sanding KSA {{ $tahunAwal }}&ndash;{{ $tahunAkhir }} Bulan {{ $namaBulan }}
            </h1>
            <p class="mt-2 text-sm text-gray-600">KSA &mdash; Data otomatis dari Sanding Bulanan &mdash; Satuan: Hektar (Ha)</p>
        </div>
        <a href="{{ route('user.ksa.panen.export', ['bulan' => $bulan, 'tahun_awal' => $tahunAwal, 'tahun_akhir' => $tahunAkhir]) }}"
        ...
        <div class="bg-white rounded-xl shadow-md p-6 border border-gray-200">
            <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                <div>
                    ...
                    <select name="bulan" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-violet-500 focus:ring-2 focus:ring-violet-200 transition-all">
                ...
                <div class="flex items-end gap-2">
                    ...
                    @if($kabupatenId || $bulan != array_key_first($bulanList) || $tahunAwal != min($availableYears) || $tahunAkhir != max($availableYears))
                    <a href="{{ route('user.ksa.panen.index') }}" class="px-4 py-2.5 bg-gray-100 text-gray-600 rounded-lg hover:bg-gray-200 font-semibold text-sm transition-all whitespace-nowrap">Reset</a>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="bg-gradient-to-r from-violet-50 to-indigo-50 border-l-4 border-violet-500 p-4 mb-6 rounded-r-lg shadow-md">
        <div class="flex items-center gap-3">
            <svg class="h-6 w-6 text-violet-600 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"/></svg>
            <p class="text-sm text-violet-800 font-medium">
                <strong>Sanding Tahunan per Bulan</strong> &mdash; Data otomatis dari <a href="{{ route('user.ksa.panen.bulanan.index') }}" class="underline font-bold">Sanding Bulanan</a>.
                Bulan: <strong>{{ $namaBulan }}</strong> &bull; Periode: {{ $tahunAwal }}&ndash;{{ $tahunAkhir }} &bull; Satuan: Ha
            </p>
        </div>
    </div>
            ...
            <table class="min-w-full text-xs border-collapse">
                <thead>
                    <tr class="bg-gradient-to-r from-violet-600 to-indigo-600 text-white">
                        <th class="border border-violet-700 px-2 py-2.5 text-center font-bold" rowspan="2">No</th>
                        <th class="border border-violet-700 px-3 py-2.5 text-center font-bold min-w-[160px]" rowspan="2">Kabupaten/Kota</th>
                        <th class="border border-violet-700 px-4 py-2 text-center font-bold" colspan="{{ count($years) }}">Luas Panen (Ha) &mdash; {{ $namaBulan }}</th>
                        <th class="border border-violet-700 px-3 py-2.5 text-center font-bold min-w-[100px]" rowspan="2">Total</th>
                    </tr>
                    <tr class="bg-gradient-to-r from-violet-500 to-indigo-500 text-white text-xs">
                        @foreach($years as $year)
                        <th class="border border-violet-600 px-2 py-2 text-center font-bold min-w-[72px]">{{ $year }}</th>
                        @endforeach
                    </tr>
                </thead>
                <tbody class="bg-white">
                    @forelse($groupedData as $index => $data)
                    <tr class="hover:bg-violet-50 transition-colors duration-150 {{ $index % 2 == 0 ? 'bg-gray-50' : 'bg-white' }}">
                        <td class="border border-gray-300 px-2 py-2.5 text-center font-semibold text-gray-700">{{ $index + 1 }}</td>
                        ...
                        @endforeach
                        <td class="border border-violet-200 px-3 py-2.5 text-right font-bold {{ $data['total'] > 0 ? 'text-violet-700 bg-violet-50' : 'text-gray-300' }}">
                            {{ ($data['total'] ?? 0) == 0 ? '0' : (floor($data['total'] ?? 0) == ($data['total'] ?? 0) ? number_format($data['total'] ?? 0, 0, ',', '.') : number_format($data['total'] ?? 0, 2, ',', '.')) }}
                        </td>
                    </tr>
                    @empty
                    <tr><td colspan="{{ count($years) + 3 }}" class="border border-gray-300 text-center py-12 text-gray-500 font-semibold">Belum ada data untuk bulan <strong>{{ $namaBulan }}</strong> periode {{ $tahunAwal }}-{{ $tahunAkhir }}</td></tr>
                    @endforelse
                </tbody>
    ...
    <div class="mt-6 grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div class="bg-gradient-to-br from-purple-500 to-purple-700 rounded-xl p-6 text-white shadow-xl">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm opacity-90">LTP Tertinggi {{ $tahunAkhir }}</p>
