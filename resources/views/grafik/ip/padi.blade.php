    ...
    <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden mb-6">
        <div class="bg-gradient-to-r from-green-500 to-emerald-600 px-6 py-4 flex items-center justify-between gap-3">
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('grafik.ip.pdf', ['tahun_awal'=>$tahunAwal,'tahun_akhir'=>$tahunAkhir]) }}" class="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold bg-white/20 text-white hover:bg-white/40 transition-all border border-white/30">
...
document.addEventListener('DOMContentLoaded',function(){
    ...
    const lp={id:'lp',afterDatasetsDraw(chart){chart.data.datasets.forEach((d,di)=>chart.getDatasetMeta(di).data.forEach((bar,idx)=>{const v=d.data[idx];if(!v||v<=0)return;const{ctx}=chart;ctx.save();ctx.font='bold 7px sans-serif';ctx.fillStyle='#111';ctx.textAlign='center';ctx.textBaseline='bottom';/* ctx.fillText removed */ ctx.restore();}));}};
    new Chart(document.getElementById('ipChart').getContext('2d'),{type:'bar',data:{labels,datasets:ds},plugins:[lp],options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{backgroundColor:'rgba(0,0,0,0.85)',callbacks:{label:ctx=>' '+ctx.dataset.label+': '+fmtFull2(ctx.parsed.y)}}},scales:{x:{border: { display: true, color: '#9CA3AF' }, grid: { display: false, drawTicks: false },ticks:{font:{size:8,weight:'600'},maxRotation:45,minRotation:45}},y:{beginAtZero: true, border: { display: true, color: '#6B7280' }, grid: { color: '#E5E7EB', drawTicks: false, borderDash: [5, 5] },max:st*8,ticks:{stepSize:st,callback:v=>new Intl.NumberFormat('id-ID',{minimumFractionDigits:2}).format(v),font:{size:10}}}}}});
});
</script>
@endsection