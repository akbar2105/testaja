        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 flex items-center gap-3">
                    ...
                    Luas Tanam Padi - {{ $months[(int)$bulan] ?? '' }} {{ $tahun }}
                </h1>
                <p class="mt-1 text-sm text-gray-600">Data harian luas tanam padi per kecamatan di Sumatera Selatan</p>
            </div>
            <div class="flex gap-2">
                <a href="{{ route('rekap.harian.tanam.export', ['tahun' => $tahun, 'bulan' => $bulan]) }}"
                ...
                <a href="{{ route('rekap.harian.tanam.create') }}"
        ...
        <div
            ...
            <div class="flex items-center">
                ...
                <p class="ml-3 text-sm text-green-800 font-medium">
                    <strong>Informasi:</strong> Total LTT = LTP Reguler + OPLAH + GOGO + CSR | Realisasi = Target - Total
                    LTT
                </p>
            </div>
        </div>
                ...
                <table class="min-w-max text-xs border-collapse">
                    <thead>
                        <tr class="bg-gradient-to-r from-green-200 to-green-300 text-gray-800 text-center font-semibold">
                            ...
                            <th class="w-[120px] min-w-[120px] border border-gray-300 px-2 py-2.5 align-middle" rowspan="2">LTT<br>Reguler</th>
                            <th class="border border-gray-300 px-2 py-2.5 align-middle" colspan="3">Total</th>
                            <th class="w-[120px] min-w-[120px] border border-gray-300 px-2 py-2.5 align-middle font-bold text-gray-900" rowspan="2">Total<br>LTT</th>
                    ...
                    <tbody>
                        ...
                        @forelse($groupedRekaps as $kabupatenId => $kabupatenRekaps)
                            @php
                                $noKabupaten++;
                                $noKecamatan = 0;
                                $kabTotals = $kabTotalsData[$kabupatenId] ?? ['target' => 0, 'total_tanam' => 0, 'oplah' => 0, 'gogo' => 0, 'csr' => 0, 'total_ltt' => 0, 'realisasi' => 0];
                            ...
                            <tr class="{{ $noKabupaten % 2 === 1 ? 'bg-gray-50' : 'bg-white' }} hover:bg-green-50 transition-colors duration-150">
                                ...
                                <td class="border border-gray-300 px-2 py-2.5 text-right font-bold text-gray-900">{{ fmtVal($kabTotals['total_ltt']) }}</td>
                            ...
                            @foreach($kabupatenRekaps as $idx => $row)
                                ...
                                <tr class="{{ $rowBg }} hover:bg-green-50 transition-colors duration-150">
                                    ...
                                    <td class="border border-gray-300 px-2 py-2.5 text-center text-gray-600">{{ $noKecamatan }}</td>
                                        ...
                                        <td class="border border-gray-300 px-2 py-2.5 text-right font-bold text-gray-900">{{ fmtVal($row->total_ltt) }}</td>
                                        ...
                                        <td class="border border-gray-300 px-2 py-2.5 text-center whitespace-nowrap">
                                            @if(isset($row->kabupaten_id) && Auth::user()->canAccessKabupaten($row->kabupaten_id))
                                                @if($row->id)
                                                    <a href="{{ route('rekap.harian.tanam.edit', $row->id) }}"
                                                    ...
                                                    <form action="{{ route('rekap.harian.tanam.destroy', $row->id) }}" method="POST"
                                                ...
                                                @else
                                                    {{-- ✅ FIX: Tambahkan kecamatan_id ke URL --}}
                                                    <a href="{{ route('rekap.harian.tanam.create') }}?kabupaten_id={{ $row->kabupaten_id }}&kecamatan_id={{ $row->kecamatan_id }}&tahun={{ $tahun }}&bulan={{ $bulan }}"
                                                       class="inline-block px-2 py-1 bg-green-500 text-white rounded text-xs hover:bg-green-600 transition-colors shadow-sm">Input</a>
                                                @endif
                                            @endif
                                        </td>
                                    </tr>
                    ...
                    @if(count($groupedRekaps) > 0)
                        <tfoot>
                            <tr class="bg-gradient-to-r from-green-500 to-green-600 text-white font-bold">
                                ...
                                @endfor
                                <td class="border border-green-400 px-2 py-2.5 text-right">
                                    {{ fmtVal($grandTotals['total_tanam']) }}</td>
                                ...
                                <td class="border border-green-400 px-2 py-2.5 text-right font-bold text-white">
                                    {{ fmtVal($grandTotals['total_ltt']) }}</td>
                                <td class="border border-green-400 px-2 py-2.5 text-right">
