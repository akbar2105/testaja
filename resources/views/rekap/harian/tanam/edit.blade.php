...
<div class="max-w-7xl mx-auto px-4 py-6">
        ...
        <div class="flex items-center justify-between mb-4">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 flex items-center gap-3">
                    ...
                    Edit Data Harian Luas Tanam
                </h1>
                <p class="mt-2 text-sm text-gray-600">Perbarui data luas tanam harian per kecamatan</p>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-xl border border-gray-200">
        <div class="bg-gradient-to-r from-yellow-500 to-yellow-600 px-6 py-4 rounded-t-xl">
            <h2 class="text-xl font-bold text-white">Form Edit Data Harian Luas Tanam</h2>
        </div>

        <form action="{{ route('rekap.harian.tanam.update', $rekap->id) }}" method="POST" id="formRekapHarian" class="p-6">
            @csrf
            @method('PUT')
            
            <div class="bg-yellow-50 border-l-4 border-yellow-500 p-4 mb-6 rounded-r-lg">
                <p class="text-sm text-yellow-800 font-medium">
                    <strong>Informasi:</strong> Total LTT = Total Tanam + OPLAH + Gogo + CSR.
                    <strong>Realisasi</strong> = Target - Total LTT
                </p>
            </div>
            ...
            <div class="mb-6">
                <div class="flex items-center justify-between mb-4 pb-2 border-b-2 border-yellow-500">
                    ...
                    <div class="text-sm">
                        <span class="font-semibold text-gray-700">Total Tanam:</span>
                        <span class="ml-2 px-3 py-1 bg-yellow-100 text-yellow-800 rounded-lg font-bold" id="totalTanamDisplay">0</span>
            ...
            <div class="mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Total LTT (Ha) <span class="text-xs text-gray-400 font-normal">(Auto)</span></label>
                        <input type="text" id="total_ltt_display" readonly value="0"
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-1" id="realisasi_formula">Target - Total LTT</p>
                    </div>

                </div>

                <div class="mt-4 bg-yellow-50 border-l-4 border-yellow-400 p-3 rounded-r-lg">
                    <p class="text-xs text-yellow-700 font-medium"><strong>Rumus:</strong> Total LTT = Total Tanam + OPLAH + Gogo + CSR | Realisasi = Target - Total LTT</p>
                </div>
            </div>
            ...
            <div class="mb-6 bg-gradient-to-r from-yellow-50 to-amber-50 rounded-xl border border-yellow-200 p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
                    <div class="bg-white rounded-lg p-4 border border-yellow-200">
                        <p class="text-gray-600 font-semibold">LTT Reguler</p>
                        <p class="text-2xl font-bold text-yellow-700 mt-1" id="summaryTotal">0</p>
                        <p class="text-xs text-gray-500 mt-1">Hektar (Ha)</p>
                    </div>
                    <div class="bg-white rounded-lg p-4 border border-yellow-200">
                        <p class="text-gray-600 font-semibold">Total LTT</p>
                        <p class="text-2xl font-bold text-purple-700 mt-1" id="summaryLTT">0</p>
                        <p class="text-xs text-gray-500 mt-1">Hektar (Ha)</p>
                    </div>
                    <div class="bg-white rounded-lg p-4 border border-yellow-200">
                    ...
                    <div class="bg-white rounded-lg p-4 border border-yellow-200">
                        <p class="text-gray-600 font-semibold">Realisasi (Target - LTT)</p>
            ...
            <div class="flex items-center justify-between pt-6 border-t border-gray-200">
                ...
                <div class="flex items-center gap-3">
                    <a href="{{ route('rekap.harian.tanam.index') }}" class="px-6 py-2.5 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-all font-semibold">Batal</a>
        ...
        <form id="deleteForm" action="{{ route('rekap.harian.tanam.destroy', $rekap->id) }}" method="POST" style="display:none;" onsubmit="return confirm('Yakin ingin menghapus data ini?');">
            @csrf
            @method('DELETE')
        </form>
    </div>
...
function hitungSemua() {
    ...
    const elTotalDisplay = document.getElementById('totalTanamDisplay');
    if(elTotalDisplay) elTotalDisplay.textContent = fmtVal(totalMain);
    
    if(document.getElementById('total_ltt_display')) document.getElementById('total_ltt_display').value = fmtVal(totalALL);
    if(document.getElementById('summaryTotal')) document.getElementById('summaryTotal').textContent = fmtVal(totalMain);
    if(document.getElementById('summaryLTT')) document.getElementById('summaryLTT').textContent = fmtVal(totalALL);
