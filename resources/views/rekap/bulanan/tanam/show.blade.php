...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="flex items-center justify-between mb-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">
                Detail Tanam Bulanan – {{ $rekapBulananTanam->kabupaten->nama_kabupaten }}
            </h1>
            <p class="text-gray-600 mt-1">
                Tahun: {{ $rekapBulananTanam->tahun }}
            </p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
            ...
            <div class="text-2xl font-bold text-blue-900">
                {{ number_format($rekapBulananTanam->januari ?? 0, 2) }}
            </div>
        ...
        <div class="bg-green-50 border border-green-200 rounded-lg p-4">
            ...
            <div class="text-2xl font-bold text-green-900">
                {{ number_format($rekapBulananTanam->februari ?? 0, 2) }}
            </div>
        ...
        <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            ...
            <div class="text-2xl font-bold text-yellow-900">
                {{ number_format($rekapBulananTanam->maret ?? 0, 2) }}
            </div>
        ...
        <div class="bg-purple-50 border border-purple-200 rounded-lg p-4">
            ...
            <div class="text-2xl font-bold text-purple-900">
                {{ number_format($rekapBulananTanam->total ?? 0, 2) }}
            </div>
    ...
    <div class="bg-white shadow rounded-lg overflow-hidden mb-6">
        <div class="px-4 py-3 bg-gray-100 border-b">
            <h3 class="text-lg font-semibold text-gray-800">
                Data Per Kecamatan - Bulan {{ $namaBulan[$bulanInt] }} {{ $rekapBulananTanam->tahun }}
            </h3>
        </div>
                    ...
                    @forelse ($harianData as $index => $row)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-right font-semibold text-gray-900">
                                {{ number_format($row->total_tanam ?? 0, 2) }}
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-8 text-center text-gray-500">
                                ...
                                <p class="text-sm">Belum ada data tanam harian untuk bulan {{ $namaBulan[$bulanInt] }}</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
                    ...
                    <tfoot class="bg-gray-100 font-bold">
                        <tr>
                            ...
                            <td class="px-6 py-4 text-right text-sm text-gray-900">
                                {{ number_format($harianData->sum('total_tanam'), 2) }}
                            </td>
                            <td class="px-6 py-4 text-right text-sm text-gray-900">
    ...
    <div class="bg-white shadow rounded-lg p-6">
        <h3 class="text-lg font-semibold text-gray-800 mb-4">
            Ringkasan Tahunan {{ $rekapBulananTanam->tahun }}
        </h3>

        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            @php
                $months = [
                    'Januari' => $rekapBulananTanam->januari,
                    'Februari' => $rekapBulananTanam->februari,
                    'Maret' => $rekapBulananTanam->maret,
                    'April' => $rekapBulananTanam->april,
                    'Mei' => $rekapBulananTanam->mei,
                    'Juni' => $rekapBulananTanam->juni,
                    'Juli' => $rekapBulananTanam->juli,
                    'Agustus' => $rekapBulananTanam->agustus,
                    'September' => $rekapBulananTanam->september,
                    'Oktober' => $rekapBulananTanam->oktober,
                    'November' => $rekapBulananTanam->november,
                    'Desember' => $rekapBulananTanam->desember,
                ];
            @endphp
