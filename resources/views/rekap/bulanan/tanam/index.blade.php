...
<div class="max-w-full mx-auto px-4 py-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 flex items-center gap-3">
                ...
                Rekap Bulanan Luas Tanam Tahun {{ $tahun }}
            </h1>
            <p class="mt-1 text-sm text-gray-600">Data luas tanam padi per kabupaten/kota di Sumatera Selatan</p>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('user.rekap.bulanan.tanam.export', ['tahun' => $tahun]) }}"
            ...
            <table class="min-w-full text-xs border-collapse">
                <thead>
                    <tr class="bg-gradient-to-r from-green-200 to-green-300 text-gray-800 text-center font-bold">
                        ...
                        <th class="border border-gray-300 px-4 py-3 text-center font-bold" colspan="12">Luas Tanam (Ha) Tahun {{ $tahun }}</th>
                    ...
                    @forelse($rekaps as $index => $rekap)
                        <tr class="hover:bg-green-50 transition-colors duration-150 {{ $index % 2 == 0 ? 'bg-gray-50' : 'bg-white' }}">
                            ...
                            <td class="border border-gray-200 px-3 py-2.5 text-center whitespace-nowrap">
                                @if($rekap->id)
                                <a href="{{ route('user.rekap.bulanan.tanam.show', $rekap->id) }}"
                        ...
                        <tr>
                            <td colspan="16" class="border border-gray-200 text-center py-12">
                                <div class="flex flex-col items-center justify-center text-gray-500">
                                    ...
                                    <p class="text-lg font-semibold">Data luas tanam belum tersedia</p>
    ...
    <div class="mt-6 grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div class="bg-gradient-to-br from-emerald-500 to-emerald-700 rounded-xl p-6 text-white shadow-xl">
            <div class="flex items-center justify-between">
                <div><p class="text-sm opacity-90">Luas Tanam Tertinggi</p><p class="text-3xl font-bold mt-2">{{ ($rekaps->max('total') ?? 0) == 0 ? '0' : (floor($rekaps->max('total')) == $rekaps->max('total') ? number_format($rekaps->max('total'), 0, ',', '.') : number_format($rekaps->max('total'), 2, ',', '.')) }}</p><p class="text-xs mt-1 opacity-75">Hektar</p></div>
        ...
        <div class="bg-gradient-to-br from-teal-500 to-teal-700 rounded-xl p-6 text-white shadow-xl">
            <div class="flex items-center justify-between">
                <div><p class="text-sm opacity-90">Rata-rata Luas Tanam</p><p class="text-3xl font-bold mt-2">{{ ($rekaps->avg('total') ?? 0) == 0 ? '0' : (floor($rekaps->avg('total')) == $rekaps->avg('total') ? number_format($rekaps->avg('total'), 0, ',', '.') : number_format($rekaps->avg('total'), 2, ',', '.')) }}</p><p class="text-xs mt-1 opacity-75">Hektar</p></div>
        ...
        <div class="bg-gradient-to-br from-lime-500 to-lime-700 rounded-xl p-6 text-white shadow-xl">
            <div class="flex items-center justify-between">
                <div><p class="text-sm opacity-90">Total Luas Tanam</p><p class="text-3xl font-bold mt-2">{{ ($totals->grand_total ?? 0) == 0 ? '0' : (floor($totals->grand_total) == $totals->grand_total ? number_format($totals->grand_total, 0, ',', '.') : number_format($totals->grand_total, 2, ',', '.')) }}</p><p class="text-xs mt-1 opacity-75">Hektar</p></div>
