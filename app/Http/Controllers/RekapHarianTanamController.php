...
use App\Exports\RekapHarianTanamExport;
...
class RekapHarianTanamController extends Controller
{
    // ── Helpers ───────────────────────────────────────────────────────────────
    ...
    {
        $dbYears = RekapHarianTanam::selectRaw('YEAR(tanggal) as year')
        ...
        $rekaps = $allKecamatan->map(function ($kec) use ($rekapsData) {
            $row = $rekapsData->get($kec->id) ?? new RekapHarianTanam(['kabupaten_id' => $kec->kabupaten_id, 'kecamatan_id' => $kec->id]);
            $row->id = $row->id ?? null;
        ...
        return view('rekap.harian.tanam.index', compact(
    ...
    {
        /** @var array<string, float|int> $grandTotals */
        $grandTotals = ['target' => 0, 'total_tanam' => 0, 'oplah' => 0, 'gogo' => 0, 'csr' => 0, 'total_ltt' => 0, 'realisasi' => 0];
        for ($d = 1; $d <= 31; $d++) $grandTotals["tgl_$d"] = 0;

        /** @var array<int, array> $kabTotalsData */
        $kabTotalsData = [];
        foreach ($kabupatens as $kab) {
            $kabTotalsData[$kab->id] = ['target' => 0, 'total_tanam' => 0, 'oplah' => 0, 'gogo' => 0, 'csr' => 0, 'total_ltt' => 0, 'realisasi' => 0];
        ...
        foreach ($rekapsData as $row) {
            ...
            $realisasi = $row->target - $row->total_ltt;

            $kabTotalsData[$kabId]['target'] += $row->target;
            $kabTotalsData[$kabId]['total_tanam'] += $row->total_tanam;
            $kabTotalsData[$kabId]['oplah'] += $row->oplah;
            $kabTotalsData[$kabId]['gogo'] += $row->gogo;
            $kabTotalsData[$kabId]['csr'] += $row->csr;
            $kabTotalsData[$kabId]['total_ltt'] += $row->total_ltt;
            $kabTotalsData[$kabId]['realisasi'] += $realisasi;

            $grandTotals['target'] += $row->target;
            $grandTotals['total_tanam'] += $row->total_tanam;
            $grandTotals['oplah'] += $row->oplah;
            $grandTotals['gogo'] += $row->gogo;
            $grandTotals['csr'] += $row->csr;
            $grandTotals['total_ltt'] += $row->total_ltt;
            $grandTotals['realisasi'] += $realisasi;
    ...
    public function create(Request $request)
    {
        ...
        // Jika dibatasi: langsung load kecamatan kabupatennya
        ...
        return view('rekap.harian.tanam.create', compact(
            'kabupatens',
            'kecamatans',
            'defaultKabupatenId',
            'reqKabupatenId',
    ...
    public function store(Request $request)
    {
        ...
        $exists = RekapHarianTanam::where('kecamatan_id', $validated['kecamatan_id'])
        ...
        RekapHarianTanam::create($dataToInsert);

        return redirect()->route('rekap.harian.tanam.index')
            ->with('success', 'Data harian berhasil disimpan! Total LTT: ' . number_format($totalLtt, 2) . ' Ha');
    }
    ...
    {
        $rekap = RekapHarianTanam::with(['kabupaten', 'kecamatan'])->findOrFail($id);
        ...
        return view('rekap.harian.tanam.edit', compact('rekap', 'kabupatens', 'kecamatans'));
    }
    ...
    {
        $rekap = RekapHarianTanam::findOrFail($id);
        ...
        return redirect()->route('rekap.harian.tanam.index')
            ->with('success', 'Data berhasil diperbarui! Total LTT: ' . number_format($totalLtt, 2) . ' Ha');
    }
    ...
    public function destroy($id)
    {
        ...
        if (str_contains(url()->previous(), '/edit')) {
            return redirect()->route('rekap.harian.tanam.index')->with('success', 'Data berhasil dihapus!');
        }
    ...
    public function getKecamatan(Request $request)
    {
        ...
        // Jika dibatasi, abaikan kabupaten_id dari request
    ...
    public function export(Request $request)
    {
        ...
        $export = new RekapHarianTanamExport($tahun, $bulan);
