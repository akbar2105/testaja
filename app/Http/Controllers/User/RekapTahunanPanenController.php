...
use App\Exports\RekapTahunanPanenExport;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use Maatwebsite\Excel\Facades\Excel;

class RekapTahunanPanenController extends Controller
{
    public function index(Request $request)
    {
        // ── Semua tahun yang ada di DB ──────────────────────────────────────
        $tahunList = RekapTahunanPanen::select('tahun')
            ->distinct()
            ->orderBy('tahun')
            ->pluck('tahun');
        ...
        $data = RekapTahunanPanen::whereIn('tahun', $tahunDipilih)
            ->when($kabupatenId, function ($q) use ($kabupatenId) {
        ...
        $totalPerTahun = RekapTahunanPanen::whereIn('tahun', $tahunDipilih)
            ->when($kabupatenId, function ($q) use ($kabupatenId) {
                return $q->where('kabupaten_id', $kabupatenId);
            })
        ...
        return view('user.rekap.tahunan.panen.index', compact(
            'kabupatens',
            'semuaKabupaten',
            'kabupatenId',
            'tahunList',
    ...
    public function export(Request $request)
    {
        ...
        $filename     = 'rekap_tahunan_panen_' . $tahunAwal . '-' . $tahunAkhir . '.xlsx';

        return Excel::download(new RekapTahunanPanenExport($tahunDipilih), $filename);
    }
}