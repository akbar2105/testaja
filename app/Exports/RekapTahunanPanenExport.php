...
/**
 * RekapTahunanPanenExport
 *
 * FIX:
 *  - Header "Total LTP Januari – Desember" → merge C3 s.d. kolom tahun terakhir ($lastCol)
 *  - Footer "SUMATERA SELATAN" → merge A:B, label di kolom A array data
 */
class RekapTahunanPanenExport implements FromArray, WithTitle, WithStyles, WithColumnWidths
{
    ...
    public function array(): array
    {
        ...
        $data          = RekapTahunanPanen::whereIn('tahun', $this->tahunList)->get()->groupBy('kabupaten_id');
        $totalPerTahun = RekapTahunanPanen::whereIn('tahun', $this->tahunList)
        ...
        // Baris 1 — Judul
        $rows[] = array_merge(
            ['REKAP TAHUNAN LUAS PANEN PADI'],
            array_fill(0, $nT + 1, '')
        );

        // Baris 2 — Sub judul
        $rows[] = array_merge(
            ['Sanding Total LTP Padi Januari–Desember per Kabupaten/Kota di Sumatera Selatan'],
            array_fill(0, $nT + 1, '')
        );

        // Baris 3 — Header baris-1 (akan di-merge di styles)
        $h1 = ['No', 'Kabupaten/Kota'];
        for ($i = 0; $i < $nT; $i++) $h1[] = 'Total LTP Januari – Desember';
        $h1[] = ''; // Untuk TOTAL
        $rows[] = $h1;
        ...
        // Baris footer — SUMATERA SELATAN (A = label, B = kosong, C+ = nilai)
        $rowTotal = ['Total', ''];
        $grandTotalInfo = 0;
        foreach ($this->tahunList as $t) {
    ...
    public function styles(Worksheet $sheet): array
    {
        ...
        $lastTahunCol = Coordinate::stringFromColumnIndex($lastColI - 1);
        ...
        // FIX: "Total LTP Januari – Desember" merge C3 sampai lastCol (kolom tahun terakhir)
        if ($nT >= 1) {
