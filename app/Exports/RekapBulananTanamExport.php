...
class RekapBulananTanamExport
{
    ...
    // ── Field nama bulan di model RekapBulananTanam ───────────────────────────
    // Sesuaikan dengan nama kolom di tabel rekap_bulanan_tanam
    ...
    /** Baca nilai numerik dari model secara aman (handle string|null dari cast decimal). */
    ...
    private function preloadAll(): void
    {
        ...
        // Rekap bulanan berdasarkan tahun yang dipilih
        $this->bulananCache = RekapBulananTanam::where('tahun', $this->tahun)
            ->get()->keyBy('kabupaten_id');

        // Semua data harian tahun ini dalam 1 query
        $allHarian = RekapHarianTanam::with(['kabupaten', 'kecamatan'])
    ...
    private function buildBulananRows(): Collection
    {
        return $this->kabupatenList->map(function (Kabupaten $kab) {
            ...
            if (!$row) {
                $row = new RekapBulananTanam(['kabupaten_id' => $kab->id, 'tahun' => $this->tahun]);
            }
    ...
    private function buildHarianRows(int $bulanNum): Collection
    {
        ...
        return $this->kecamatanList->map(function (Kecamatan $kec) use ($harianByKec) {
            ...
            $empty = new RekapHarianTanam(['kabupaten_id' => $kec->kabupaten_id, 'kecamatan_id' => $kec->id]);
    ...
    {
        $lc = self::LAST_COL;
        $sheet->setCellValue('A1', "REKAP BULANAN LUAS TAMBAH TANAM {$this->tahun}");
        ...
        $sheet->setCellValue('A2', 'Sanding Luas Tanam Padi per Kabupaten/Kota di Sumatera Selatan');
    ...
    private function writeRekapHeader(Worksheet $sheet): void
    {
        ...
        $sheet->setCellValue('C3', "LUAS TANAM (Ha) TAHUN {$this->tahun}");
    ...
    private function writeRekapDataRows(Worksheet $sheet, Collection $rekaps): int
    {
        ...
        foreach ($rekaps as $rekap) {
            ...
            $rowTotal = 0.0;
            foreach (self::BULAN_COLS as $bNum => $col) {
                // Ambil dari field nama bulan model
    ...
    private function writeRekapJumlahRow(Worksheet $sheet, int $dataEnd): int
    {
        ...
        foreach (self::BULAN_COLS as $bNum => $col) {
            ...
            $totalSum += $colSum;
            $sheet->setCellValue("{$col}{$jRow}", "=SUM({$col}5:{$col}{$dataEnd})");
            if (fmod(abs($colSum), 1.0) >= 1e-9) $decCells[] = "{$col}{$jRow}";
        }
        $sheet->setCellValue("O{$jRow}", "=SUM(O5:O{$dataEnd})");
    ...
    private function writeRekapGogoRows(Worksheet $sheet, int $jRow): array
    {
        ...
        foreach (self::BULAN_COLS as $bNum => $col) {
            $this->writeNum($sheet, "{$col}{$gogoRow}", $this->gogoSumCache[$bNum] ?? 0.0);
            $this->writeNum($sheet, "{$col}{$oplahRow}", $this->oplahSumCache[$bNum] ?? 0.0);
            $this->writeNum($sheet, "{$col}{$csrRow}", $this->csrSumCache[$bNum] ?? 0.0);
        }
        $sheet->setCellValue("O{$gogoRow}", "=SUM(C{$gogoRow}:N{$gogoRow})");
        $sheet->setCellValue("O{$oplahRow}", "=SUM(C{$oplahRow}:N{$oplahRow})");
        $sheet->setCellValue("O{$csrRow}", "=SUM(C{$csrRow}:N{$csrRow})");
        ...
        $this->applyNumberFormat($sheet, "C{$gogoRow}:{$lc}{$csrRow}");
    ...
    private function writeRekapTotalRow(Worksheet $sheet, int $jRow, int $gogoRow, int $oplahRow, int $csrRow): void
    {
        ...
        foreach (self::BULAN_COLS as $bNum => $col) {
            ...
            $totalSum += $colTotal;
            $sheet->setCellValue("{$col}{$totalRow}", "={$col}{$jRow}+{$col}{$gogoRow}+{$col}{$oplahRow}+{$col}{$csrRow}");
            if (fmod(abs($colTotal), 1.0) >= 1e-9) $decCells[] = "{$col}{$totalRow}";
        }
        $sheet->setCellValue("O{$totalRow}", "=SUM(O{$jRow}:O{$csrRow})");
    ...
    private function buildLuasTanamSheet(Spreadsheet $spreadsheet, int $bulanNum, string $bulanName): void
    {
        $sheet = $spreadsheet->createSheet();
        $sheet->setTitle('LUAS TANAM ' . substr($bulanName, 0, 3));
        $this->setDailyColumnWidths($sheet);
        $this->writeDailyTitle($sheet, $bulanName);
        $this->writeDailyHeader($sheet, 'LTT REGULER', 'TOTAL LTT');
    ...
    private function writeDailyTitle(Worksheet $sheet, string $bulanName): void
    {
        ...
        $sheet->setCellValue("{$A}1", "LUAS TANAM PADI — BULAN {$bulanName} {$this->tahun}");
    ...
    private function writeDailyTotalRow(Worksheet $sheet, int $row, array $jumlahRows): void
    {
        ...
        $this->applyNumberFormat($sheet, "{$E}{$row}:{$AP}{$row}");
    ...
    public function download(): never
    {
        ...
        foreach (self::BULAN_NAMES as $num => $name) {
            $this->buildLuasTanamSheet($spreadsheet, $num, $name);
        }
        ...
        header('Content-Disposition: attachment;filename="rekaptanam-' . $this->tahun . '.xlsx"');
