...
class KsaProduksiTotalExport implements FromArray, WithStyles, WithColumnWidths, WithTitle
{
    protected int $tahunAwal;
    protected int $tahunAkhir;

    const COLOR_HEADER_BG   = '2563EB';
    const COLOR_SUBHDR_BG   = '1D4ED8';
    const COLOR_FOOTER_BG   = '1E40AF';
    const COLOR_FOOTER_CELL = '1E3A8A';
    ...
    {
        return "Total {$this->tahunAwal}-{$this->tahunAkhir}";
    }
    ...
    public function array(): array
    {
        ...
        $rows = [];
        $rows[] = array_merge(["KSA PRODUKSI PADI (TON GKG) — SANDING TOTAL TAHUNAN"], array_fill(0, count($years) + 2, ''));
        $rows[] = array_merge(["Periode: {$tahunAwal}–{$tahunAkhir}   |   Satuan: Ton GKG"], array_fill(0, count($years) + 2, ''));
        $rows[] = array_fill(0, count($years) + 3, '');

        $header1 = ['No', 'Kabupaten/Kota', "Total Produksi Padi (Ton GKG)"];
        ...
        foreach ($kabupatens as $i => $kab) {
            ...
            $kabTotal = 0;
            foreach ($years as $y) {
                $val = (float) KsaProduksi::where('kabupaten_id', $kab->id)->where('tahun', $y)->sum('produksi');
        ...
        $rows[] = [];
        $rows[] = ["* Satuan: Ton GKG   |   Tahun: {$tahunAwal}–{$tahunAkhir}"];
    ...
    public function styles(Worksheet $sheet): array
    {
        ...
        $dataRowStart = 6;
        $dataRowEnd   = $dataRowStart + $kabupatens - 1;
