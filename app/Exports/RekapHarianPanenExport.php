...
private function applyNumberFormat(Worksheet $sheet, string $range): void
    ...
    private function buildSpreadsheet(): Spreadsheet
    {
        ...
        $sheet->setTitle("LUAS PANEN {$this->bulanName}");
    ...
    {
        $A = $this->colA(); $AP = $this->colAP();

        $sheet->setCellValue("{$A}1", "LUAS PANEN PADI — BULAN {$this->bulanName} {$this->tahun}");
    ...
    private function writeHeaderRows(Worksheet $sheet): void
    {
        ...
        $sheet->setCellValue("{$AK}3", 'LTP REGULER');
        $sheet->setCellValue("{$AL}3", 'TOTAL');
        $sheet->setCellValue("{$AO}3", 'TOTAL LTP');
        $sheet->setCellValue("{$AP}3", 'Realisasi - Target');

        $sheet->mergeCells("{$A}3:{$A}4"); $sheet->mergeCells("{$B}3:{$B}4");
        $sheet->mergeCells("{$C}3:{$C}4"); $sheet->mergeCells("{$D}3:{$D}4");
        $sheet->mergeCells("{$E}3:{$E}4"); $sheet->mergeCells("{$F}3:{$AJ}3");
        $sheet->mergeCells("{$AK}3:{$AK}4"); $sheet->mergeCells("{$AL}3:{$AN}3");
        $sheet->mergeCells("{$AO}3:{$AO}4"); $sheet->mergeCells("{$AP}3:{$AP}4");
    ...
    private function writeKecamatanRow(Worksheet $sheet, RekapHarianPanen $r, int $row, int $noKec): void
    {
        [$A,$B,$C,$D,$E,$F,$AJ,$AK,$AL,$AM,$AN,$AO,$AP] = $this->cols();
...
$sheet->setCellValue("{$AO}{$row}", "={$AK}{$row}+{$AL}{$row}+{$AM}{$row}+{$AN}{$row}");
    ...
    private function writeTotalRow(Worksheet $sheet, int $row, array $jumlahRows): void
    {
        ...
        $sheet->setCellValue("{$A}{$row}", 'SUMATERA SELATAN');
    ...
    /**
     * Returns all column letter shortcuts in a consistent order.
     * @return string[]
     */
    ...
    public function download(): never
    {
        ...
        $filename = "Luas_Panen_{$this->bulanName}_{$this->tahun}.xlsx";
